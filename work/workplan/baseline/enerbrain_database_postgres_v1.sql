-- EnerBrain Database V1
-- 작성 목적:
-- 1) SITE > BIZ(프로젝트) > JOB(태스크) 중심 구조
-- 2) 물리 FK 미사용, 의미적 FK 컬럼/인덱스 기반
-- 3) 프로젝트별 API 키 + Open API 키 분리 관리

CREATE SCHEMA IF NOT EXISTS enerbrain;
SET search_path TO enerbrain, public;
...
CREATE TABLE IF NOT EXISTS TB_COMM_CD (
    ...
    SORT_ORD            NUMERIC(10,0) NOT NULL,                           -- 정렬순서
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,     -- 생성일시
    REG_USER_ID         NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 생성자
    MOD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,     -- 수정일시
    MOD_USER_ID         NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 수정자
    DEL_YN              CHAR(1) NOT NULL DEFAULT 'N' CHECK (DEL_YN IN ('Y','N')), -- 삭제여부
    PRIMARY KEY (TYPE_CD, GRP_CD, CD_ID)
);
CREATE INDEX IF NOT EXISTS IX_TB_COMM_CD_01 ON TB_COMM_CD (GRP_CD);

-- TB_COMM_CD 수정일시 자동 갱신 트리거 (PostgreSQL)
CREATE OR REPLACE FUNCTION FN_SET_TB_COMM_CD_MOD_DT()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS TRG_SET_TB_COMM_CD_MOD_DT ON TB_COMM_CD;
CREATE TRIGGER TRG_SET_TB_COMM_CD_MOD_DT
BEFORE UPDATE ON TB_COMM_CD
FOR EACH ROW
EXECUTE FUNCTION FN_SET_TB_COMM_CD_MOD_DT();
...
CREATE TABLE IF NOT EXISTS TB_BIZ (
    ...
    BIZ_BGNG_DT         TIMESTAMP,                                        -- 사업시작일시
    BIZ_END_DT          TIMESTAMP,                                        -- 사업종료일시
...
CREATE INDEX IF NOT EXISTS IX_TB_BIZ_01 ON TB_BIZ (SITE_NO);
...
CREATE TABLE IF NOT EXISTS TB_BIZ_MBR (
    ...
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (BIZ_NO, USER_NO, ROLE_SE_CD)
);
CREATE INDEX IF NOT EXISTS IX_TB_BIZ_MBR_01 ON TB_BIZ_MBR (BIZ_NO);
CREATE INDEX IF NOT EXISTS IX_TB_BIZ_MBR_02 ON TB_BIZ_MBR (USER_NO);
...
CREATE TABLE IF NOT EXISTS TB_JOB (
    ...
    JOB_CNFG_CN         JSONB,                                            -- 작업설정내용(JSON)
    JOB_BGNG_DT         TIMESTAMP,                                        -- 작업시작일시
    JOB_END_DT          TIMESTAMP,                                        -- 작업종료일시
...
CREATE INDEX IF NOT EXISTS IX_TB_JOB_01 ON TB_JOB (BIZ_NO);
CREATE INDEX IF NOT EXISTS IX_TB_JOB_02 ON TB_JOB (JOB_SE_CD, JOB_STTS_CD);
...
CREATE TABLE IF NOT EXISTS TB_JOB_RUN (
    ...
    RUN_BGNG_DT         TIMESTAMP NOT NULL,                               -- 실행시작일시
    RUN_END_DT          TIMESTAMP,                                        -- 실행종료일시
...
CREATE INDEX IF NOT EXISTS IX_TB_JOB_RUN_01 ON TB_JOB_RUN (JOB_NO);
CREATE INDEX IF NOT EXISTS IX_TB_JOB_RUN_02 ON TB_JOB_RUN (RUN_STTS_CD, RUN_BGNG_DT);
...
CREATE TABLE IF NOT EXISTS TB_DATA_SRC (
    ...
    CONN_CN             JSONB NOT NULL,                                   -- 연결내용(JSON)
...
CREATE INDEX IF NOT EXISTS IX_TB_DATA_SRC_01 ON TB_DATA_SRC (BIZ_NO);
CREATE INDEX IF NOT EXISTS IX_TB_DATA_SRC_02 ON TB_DATA_SRC (SRC_SE_CD);
...
CREATE TABLE IF NOT EXISTS TB_CLCT_JOB (
    ...
    LAST_SUCC_CLCT_DT   TIMESTAMP,                                        -- 마지막성공수집일시
    RETRY_CNT           NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 재시도횟수
    STTS_CD             VARCHAR(20),                                      -- 상태코드
    USE_YN              CHAR(1) NOT NULL DEFAULT 'Y' CHECK (USE_YN IN ('Y','N')),
    DEL_YN              CHAR(1) NOT NULL DEFAULT 'N' CHECK (DEL_YN IN ('Y','N')),
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS IX_TB_CLCT_JOB_01 ON TB_CLCT_JOB (DATA_SRC_NO);
...
CREATE TABLE IF NOT EXISTS TB_CLCT_RUN (
    ...
    CLCT_BGNG_DT        TIMESTAMP NOT NULL,                               -- 수집시작일시
    CLCT_END_DT         TIMESTAMP,                                        -- 수집종료일시
    CLCT_STTS_CD        VARCHAR(20) NOT NULL,                             -- 수집상태코드
    CLCT_NOCS           NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 수집건수
    FAIL_NOCS           NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 실패건수
    ERR_CN              VARCHAR(4000),                                    -- 오류내용
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_CLCT_RUN_01 ON TB_CLCT_RUN (CLCT_JOB_NO);
CREATE INDEX IF NOT EXISTS IX_TB_CLCT_RUN_02 ON TB_CLCT_RUN (CLCT_STTS_CD, CLCT_BGNG_DT);
...
CREATE TABLE IF NOT EXISTS TB_MAPP_RULE (
    ...
    CURR_VER_NO         NUMERIC(10,0) NOT NULL DEFAULT 1,                 -- 현재버전번호
    USE_YN              CHAR(1) NOT NULL DEFAULT 'Y' CHECK (USE_YN IN ('Y','N')),
    DEL_YN              CHAR(1) NOT NULL DEFAULT 'N' CHECK (DEL_YN IN ('Y','N')),
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_MAPP_RULE_01 ON TB_MAPP_RULE (DATA_SRC_NO);
...
CREATE TABLE IF NOT EXISTS TB_MAPP_RULE_VER (
    ...
    VER_NO              NUMERIC(10,0) NOT NULL,                           -- 버전번호
    RULE_CN             JSONB NOT NULL,                                   -- 규칙내용(JSON)
    APLCN_BGNG_DT       TIMESTAMP,                                        -- 적용시작일시
    APLCN_END_DT        TIMESTAMP,                                        -- 적용종료일시
    STTS_CD             VARCHAR(20),                                      -- 상태코드
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (MAPP_RULE_NO, VER_NO)
);
CREATE INDEX IF NOT EXISTS IX_TB_MAPP_RULE_VER_01 ON TB_MAPP_RULE_VER (MAPP_RULE_NO);
...
CREATE TABLE IF NOT EXISTS TB_QLTY_RULE (
    ...
    QLTY_RULE_CN        JSONB NOT NULL,                                   -- 품질규칙내용(JSON)
    USE_YN              CHAR(1) NOT NULL DEFAULT 'Y' CHECK (USE_YN IN ('Y','N')),
    DEL_YN              CHAR(1) NOT NULL DEFAULT 'N' CHECK (DEL_YN IN ('Y','N')),
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_QLTY_RULE_01 ON TB_QLTY_RULE (MAPP_RULE_VER_NO);
...
CREATE TABLE IF NOT EXISTS TB_QLTY_RSLT (
    ...
    CHK_NOCS            NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 검사건수
    FAIL_NOCS           NUMERIC(10,0) NOT NULL DEFAULT 0,                 -- 실패건수
    QLTY_STTS_CD        VARCHAR(20) NOT NULL,                             -- 품질상태코드
    RSLT_CN             VARCHAR(4000),                                    -- 결과내용
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_QLTY_RSLT_01 ON TB_QLTY_RSLT (CLCT_RUN_NO);
CREATE INDEX IF NOT EXISTS IX_TB_QLTY_RSLT_02 ON TB_QLTY_RSLT (QLTY_RULE_NO);
...
CREATE INDEX IF NOT EXISTS IX_TB_RTU_01 ON TB_RTU (SITE_NO, BIZ_NO);
CREATE INDEX IF NOT EXISTS IX_TB_RTU_02 ON TB_RTU (RTU_SE_CD, STTS_CD);
...
CREATE INDEX IF NOT EXISTS IX_TB_METER_01 ON TB_METER (SITE_NO, BIZ_NO, RTU_NO);
CREATE INDEX IF NOT EXISTS IX_TB_METER_02 ON TB_METER (METER_SE_CD, STTS_CD);
...
CREATE TABLE IF NOT EXISTS TB_TS_RAW_JSON (
    ...
    SRC_TS_DT            TIMESTAMP,                                       -- 원천기준일시
    PAYLOAD_CN           JSONB NOT NULL,                                  -- 원천페이로드내용
    MAPP_RULE_VER_NO     VARCHAR(20),                                     -- 매핑규칙버전번호
    PRCS_STTS_CD         VARCHAR(20) NOT NULL,                            -- 처리상태코드
    ERR_CN               VARCHAR(4000),                                   -- 오류내용
    REG_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_TS_RAW_JSON_01 ON TB_TS_RAW_JSON (SITE_NO, BIZ_NO, SRC_TS_DT);
CREATE INDEX IF NOT EXISTS IX_TB_TS_RAW_JSON_02 ON TB_TS_RAW_JSON (DATA_SRC_NO, PRCS_STTS_CD);
CREATE INDEX IF NOT EXISTS IX_TB_TS_RAW_JSON_03 ON TB_TS_RAW_JSON (RTU_NO, SRC_TS_DT);
CREATE INDEX IF NOT EXISTS IX_TB_TS_RAW_JSON_04 ON TB_TS_RAW_JSON (METER_NO, SRC_TS_DT);
...
CREATE TABLE IF NOT EXISTS TB_TS_FACT (
    ...
    TS_DT                TIMESTAMP NOT NULL,                              -- 기준일시
    METRIC_CD            VARCHAR(50) NOT NULL,                            -- 측정지표코드
    VAL                  NUMERIC(18,6) NOT NULL,                          -- 측정값
    UNIT_CD              VARCHAR(20),                                     -- 단위코드
    QLTY_STTS_CD         VARCHAR(20),                                     -- 품질상태코드
    TS_RAW_NO            VARCHAR(20),                                     -- 원천시계열번호
    REG_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (SITE_NO, BIZ_NO, RTU_NO, METER_NO, TS_DT, METRIC_CD)
);
CREATE INDEX IF NOT EXISTS IX_TB_TS_FACT_01 ON TB_TS_FACT (SITE_NO, BIZ_NO, TS_DT);
CREATE INDEX IF NOT EXISTS IX_TB_TS_FACT_02 ON TB_TS_FACT (BIZ_NO, RTU_NO, TS_DT);
CREATE INDEX IF NOT EXISTS IX_TB_TS_FACT_03 ON TB_TS_FACT (METRIC_CD, TS_DT);
CREATE INDEX IF NOT EXISTS IX_TB_TS_FACT_04 ON TB_TS_FACT (METER_NO, TS_DT);
...
CREATE TABLE IF NOT EXISTS TB_FCST_RSLT (
    ...
    FCST_TS_DT           TIMESTAMP NOT NULL,                              -- 예측기준일시
    METRIC_CD            VARCHAR(50) NOT NULL,                            -- 예측지표코드
    FCST_VAL             NUMERIC(18,6) NOT NULL,                          -- 예측값
    LOWER_VAL            NUMERIC(18,6),                                   -- 하한값
    UPPER_VAL            NUMERIC(18,6),                                   -- 상한값
    REG_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_FCST_RSLT_01 ON TB_FCST_RSLT (SITE_NO, BIZ_NO, TARGET_YMD);
CREATE INDEX IF NOT EXISTS IX_TB_FCST_RSLT_02 ON TB_FCST_RSLT (BIZ_NO, RTU_NO, FCST_TS_DT);
CREATE INDEX IF NOT EXISTS IX_TB_FCST_RSLT_03 ON TB_FCST_RSLT (MODEL_VER_NO, METRIC_CD);
CREATE INDEX IF NOT EXISTS IX_TB_FCST_RSLT_04 ON TB_FCST_RSLT (METER_NO, FCST_TS_DT);

-- =====================================================================
-- 03-2. PostgreSQL COMMENT 메타데이터
-- =====================================================================

COMMENT ON TABLE TB_COMM_CD IS '시스템공통코드';
COMMENT ON COLUMN TB_COMM_CD.TYPE_CD IS '물리 테이블명 또는 COMM_CD';
COMMENT ON COLUMN TB_COMM_CD.GRP_CD IS '물리 컬럼명';
COMMENT ON COLUMN TB_COMM_CD.CD_ID IS '코드ID';
COMMENT ON COLUMN TB_COMM_CD.CD_NM IS '코드명';
COMMENT ON COLUMN TB_COMM_CD.EXPLN IS '코드 설명';
COMMENT ON COLUMN TB_COMM_CD.SORT_ORD IS '정렬순서';
COMMENT ON COLUMN TB_COMM_CD.REG_DT IS '생성일시';
COMMENT ON COLUMN TB_COMM_CD.REG_USER_ID IS '생성자';
COMMENT ON COLUMN TB_COMM_CD.MOD_DT IS '수정일시';
COMMENT ON COLUMN TB_COMM_CD.MOD_USER_ID IS '수정자';
COMMENT ON COLUMN TB_COMM_CD.DEL_YN IS '삭제여부';

COMMENT ON TABLE TB_BIZ_API_KEY IS '프로젝트별 API 인증키';
COMMENT ON COLUMN TB_BIZ_API_KEY.BIZ_API_KEY_NO IS '프로젝트API키번호';
COMMENT ON COLUMN TB_BIZ_API_KEY.SITE_NO IS '사이트번호';
COMMENT ON COLUMN TB_BIZ_API_KEY.BIZ_NO IS '사업번호';
COMMENT ON COLUMN TB_BIZ_API_KEY.KEY_NM IS '키명';
COMMENT ON COLUMN TB_BIZ_API_KEY.KEY_HASH_CN IS '키해시내용';
COMMENT ON COLUMN TB_BIZ_API_KEY.KEY_PREFIX_CN IS '키접두내용';
COMMENT ON COLUMN TB_BIZ_API_KEY.KEY_STTS_CD IS '키상태코드';
COMMENT ON COLUMN TB_BIZ_API_KEY.ISSUE_TRGT_USER_NO IS '발급대상사용자번호';
COMMENT ON COLUMN TB_BIZ_API_KEY.ISSUE_CN IS '발급설명';
COMMENT ON COLUMN TB_BIZ_API_KEY.ISSUE_DT IS '발급일시';
COMMENT ON COLUMN TB_BIZ_API_KEY.EXPR_DT IS '만료일시';
COMMENT ON COLUMN TB_BIZ_API_KEY.LAST_USE_DT IS '최종사용일시';
COMMENT ON COLUMN TB_BIZ_API_KEY.REVOKE_DT IS '폐기일시';
COMMENT ON COLUMN TB_BIZ_API_KEY.RATE_LMT_PER_MIN IS '분당호출제한건수';
COMMENT ON COLUMN TB_BIZ_API_KEY.IP_WHTLST_CN IS '허용IP목록';
COMMENT ON COLUMN TB_BIZ_API_KEY.USE_YN IS '사용여부';
COMMENT ON COLUMN TB_BIZ_API_KEY.DEL_YN IS '삭제여부';
COMMENT ON COLUMN TB_BIZ_API_KEY.REG_DT IS '생성일시';
COMMENT ON COLUMN TB_BIZ_API_KEY.UPD_DT IS '수정일시';

COMMENT ON TABLE TB_TS_FACT IS '정규화 시계열 팩트';
COMMENT ON COLUMN TB_TS_FACT.TS_FACT_NO IS '시계열팩트번호';
COMMENT ON COLUMN TB_TS_FACT.SITE_NO IS '사이트번호';
COMMENT ON COLUMN TB_TS_FACT.BIZ_NO IS '사업번호';
COMMENT ON COLUMN TB_TS_FACT.RTU_NO IS 'RTU번호';
COMMENT ON COLUMN TB_TS_FACT.METER_NO IS '계측기번호';
COMMENT ON COLUMN TB_TS_FACT.TS_DT IS '기준일시';
COMMENT ON COLUMN TB_TS_FACT.METRIC_CD IS '측정지표코드';
COMMENT ON COLUMN TB_TS_FACT.VAL IS '측정값';
COMMENT ON COLUMN TB_TS_FACT.UNIT_CD IS '단위코드';
COMMENT ON COLUMN TB_TS_FACT.QLTY_STTS_CD IS '품질상태코드';
COMMENT ON COLUMN TB_TS_FACT.TS_RAW_NO IS '원천시계열번호';
COMMENT ON COLUMN TB_TS_FACT.REG_DT IS '생성일시';
COMMENT ON COLUMN TB_TS_FACT.UPD_DT IS '수정일시';
...
CREATE TABLE IF NOT EXISTS TB_ALGM (
    ...
    DFLT_CNFG_CN        JSONB,                                            -- 기본설정내용(JSON)
    USE_YN              CHAR(1) NOT NULL DEFAULT 'Y' CHECK (USE_YN IN ('Y','N')),
    DEL_YN              CHAR(1) NOT NULL DEFAULT 'N' CHECK (DEL_YN IN ('Y','N')),
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_ALGM_01 ON TB_ALGM (ALGM_SE_CD);
...
CREATE TABLE IF NOT EXISTS TB_MODEL_VER (
    ...
    TRN_BGNG_DT         TIMESTAMP,                                        -- 학습시작일시
    TRN_END_DT          TIMESTAMP,                                        -- 학습종료일시
    MODEL_STTS_CD       VARCHAR(20) NOT NULL,                             -- 모델상태코드
    ARTFCT_PATH_NM      VARCHAR(300),                                     -- 아티팩트경로명
    CNFG_CN             JSONB,                                            -- 모델설정내용(JSON)
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (JOB_NO, VER_NO)
);
CREATE INDEX IF NOT EXISTS IX_TB_MODEL_VER_01 ON TB_MODEL_VER (JOB_NO);
CREATE INDEX IF NOT EXISTS IX_TB_MODEL_VER_02 ON TB_MODEL_VER (ALGM_NO);
...
CREATE TABLE IF NOT EXISTS TB_MODEL_EVAL (
    ...
    EVAL_DT             TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,     -- 평가일시
    METRIC_CN           JSONB NOT NULL,                                   -- 평가지표내용(JSON)
    PASS_YN             CHAR(1) NOT NULL DEFAULT 'N' CHECK (PASS_YN IN ('Y','N')),
    EVAL_EXPLN          VARCHAR(4000),                                    -- 평가설명
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_MODEL_EVAL_01 ON TB_MODEL_EVAL (MODEL_VER_NO);
...
CREATE TABLE IF NOT EXISTS TB_DEPLOY (
    ...
    DEPLOY_BGNG_DT      TIMESTAMP NOT NULL,                               -- 배포시작일시
    DEPLOY_END_DT       TIMESTAMP,                                        -- 배포종료일시
    ROUTE_RT            NUMERIC(5,2),                                     -- 라우팅비율
    ROLLBACK_YN         CHAR(1) NOT NULL DEFAULT 'N' CHECK (ROLLBACK_YN IN ('Y','N')),
    REG_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT              TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_DEPLOY_01 ON TB_DEPLOY (MODEL_VER_NO);
CREATE INDEX IF NOT EXISTS IX_TB_DEPLOY_02 ON TB_DEPLOY (DEPLOY_STTS_CD, DEPLOY_BGNG_DT);
...
CREATE INDEX IF NOT EXISTS IX_TB_API_SVC_01 ON TB_API_SVC (SITE_NO);
CREATE INDEX IF NOT EXISTS IX_TB_API_SVC_02 ON TB_API_SVC (BIZ_NO);
CREATE INDEX IF NOT EXISTS IX_TB_API_SVC_03 ON TB_API_SVC (OPEN_API_YN, STTS_CD);
...
CREATE TABLE IF NOT EXISTS TB_BIZ_API_KEY (
    ...
    ISSUE_DT            TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,     -- 발급일시
    EXPR_DT             TIMESTAMP,                                        -- 만료일시
    LAST_USE_DT         TIMESTAMP,                                        -- 최종사용일시
    REVOKE_DT           TIMESTAMP,                                        -- 폐기일시
    RATE_LMT_PER_MIN    NUMERIC(10,0),                                    -- 분당호출제한건수
...
CREATE INDEX IF NOT EXISTS IX_TB_BIZ_API_KEY_01 ON TB_BIZ_API_KEY (SITE_NO, BIZ_NO);
CREATE INDEX IF NOT EXISTS IX_TB_BIZ_API_KEY_02 ON TB_BIZ_API_KEY (KEY_STTS_CD, EXPR_DT);
...
CREATE TABLE IF NOT EXISTS TB_BIZ_API_KEY_ISSU_HIS (
    ...
    ISSU_DT                  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,-- 발급일시
    ISSU_USER_NO             VARCHAR(20),                                 -- 발급사용자번호 (의미 FK: TB_USER.USER_NO)
    ISSU_RSN                 VARCHAR(4000),                               -- 발급사유
    REG_DT                   TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_BIZ_API_KEY_ISSU_HIS_01 ON TB_BIZ_API_KEY_ISSU_HIS (BIZ_API_KEY_NO, ISSU_DT);
...
CREATE INDEX IF NOT EXISTS IX_TB_OPEN_API_CLNT_01 ON TB_OPEN_API_CLNT (SITE_NO, CLNT_STTS_CD);
...
CREATE TABLE IF NOT EXISTS TB_OPEN_API_KEY (
    ...
    LAST_USE_DT          TIMESTAMP,                                       -- 최종사용일시
    RATE_LMT_PER_MIN     NUMERIC(10,0),                                   -- 분당호출제한건수
    DAILY_QTA_NOCS       NUMERIC(10,0),                                   -- 일별할당건수
    USE_YN               CHAR(1) NOT NULL DEFAULT 'Y' CHECK (USE_YN IN ('Y','N')),
    DEL_YN               CHAR(1) NOT NULL DEFAULT 'N' CHECK (DEL_YN IN ('Y','N')),
    REG_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_OPEN_API_KEY_01 ON TB_OPEN_API_KEY (OPEN_API_CLNT_NO);
CREATE INDEX IF NOT EXISTS IX_TB_OPEN_API_KEY_02 ON TB_OPEN_API_KEY (KEY_STTS_CD, EXPR_DT);
...
CREATE TABLE IF NOT EXISTS TB_OPEN_API_KEY_SVC (
    ...
    REG_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UPD_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (OPEN_API_KEY_NO, API_SVC_NO, AUTHRT_SE_CD)
);
CREATE INDEX IF NOT EXISTS IX_TB_OPEN_API_KEY_SVC_01 ON TB_OPEN_API_KEY_SVC (OPEN_API_KEY_NO);
CREATE INDEX IF NOT EXISTS IX_TB_OPEN_API_KEY_SVC_02 ON TB_OPEN_API_KEY_SVC (API_SVC_NO);
...
CREATE TABLE IF NOT EXISTS TB_API_REQ_LOG (
    ...
    REQ_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,    -- 요청일시
    REQ_IP_ADDR          VARCHAR(45),                                     -- 요청IP주소
    REQ_MTHD_CD          VARCHAR(10),                                     -- 요청메서드코드
    REQ_PATH_NM          VARCHAR(1000),                                   -- 요청경로명
    RES_STTS_CD          VARCHAR(10),                                     -- 응답상태코드
    PROC_MSEC            NUMERIC(10,0),                                   -- 처리밀리초
    ERR_CN               VARCHAR(4000)                                    -- 오류내용
);
CREATE INDEX IF NOT EXISTS IX_TB_API_REQ_LOG_01 ON TB_API_REQ_LOG (API_SVC_NO, REQ_DT);
CREATE INDEX IF NOT EXISTS IX_TB_API_REQ_LOG_02 ON TB_API_REQ_LOG (BIZ_API_KEY_NO, OPEN_API_KEY_NO);
...
CREATE TABLE IF NOT EXISTS TB_AUDIT_LOG (
    ...
    CHG_CN               JSONB,                                           -- 변경내용(JSON)
    REG_DT               TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS IX_TB_AUDIT_LOG_01 ON TB_AUDIT_LOG (SITE_NO, BIZ_NO, REG_DT);
CREATE INDEX IF NOT EXISTS IX_TB_AUDIT_LOG_02 ON TB_AUDIT_LOG (USER_NO, REG_DT);
...
ON CONFLICT (TYPE_CD, GRP_CD, CD_ID) DO NOTHING;
...
(
    ...
    'DB',
    '광명 에너지 집계 DB',
    '{"db_type":"postgresql","host":"10.20.30.11","port":5432,"database":"gms_energy","schema":"public","table":"cust_tbl"}'::jsonb,
    '태양광 발전/소비 원천 데이터',
    'ACTIVE',
    'Y',
    'N'
...
(
    ...
    'API',
    '광명 기상 API',
    '{"provider":"kma","endpoint":"https://api.weather.example/v1/obs"}'::jsonb,
    '일사량/기온 외생변수 데이터',
    'ACTIVE',
    'Y',
    'N'
)
ON CONFLICT (DATA_SRC_NO) DO NOTHING;

INSERT INTO TB_CLCT_JOB (
...
ON CONFLICT (CLCT_JOB_NO) DO NOTHING;

INSERT INTO TB_CLCT_RUN (
...
ON CONFLICT (CLCT_RUN_NO) DO NOTHING;

INSERT INTO TB_MAPP_RULE (
...
ON CONFLICT (MAPP_RULE_NO) DO NOTHING;

INSERT INTO TB_MAPP_RULE_VER (
...
ON CONFLICT (MAPP_RULE_VER_NO) DO NOTHING;

INSERT INTO TB_QLTY_RULE (
...
(
    ...
    'MRV_GMSC_ENE01',
    '발전/소비 음수값 점검',
    '{"checks":[{"metric_cd":"PV_GEN_KW","min":0},{"metric_cd":"LOAD_KW","min":0}]}'::jsonb,
    'Y',
    'N'
),
(
    ...
    'MRV_GMSC_WEA01',
    '기온/일사량 범위 점검',
    '{"checks":[{"metric_cd":"TEMP_C","min":-40,"max":60},{"metric_cd":"IRRADIANCE","min":0,"max":1500}]}'::jsonb,
    'Y',
    'N'
)
ON CONFLICT (QLTY_RULE_NO) DO NOTHING;

INSERT INTO TB_QLTY_RSLT (
...
ON CONFLICT (QLTY_RSLT_NO) DO NOTHING;
...
ON CONFLICT (RTU_NO) DO NOTHING;

INSERT INTO TB_METER (
...
(
    ...
    'cust_tbl:1001',
    '2026-04-23 00:00:00',
    '{"pv_kw":125.42,"load_kw":301.77,"measured_at":"2026-04-23T00:00:00+09:00"}'::jsonb,
    'MRV_GMSC_ENE01',
    'DONE',
    NULL
),
(
    ...
    'weather:2001',
    '2026-04-23 00:00:00',
    '{"irradiance":412.5,"temp_c":21.3,"obs_time":"2026-04-23T00:00:00+09:00"}'::jsonb,
    'MRV_GMSC_WEA01',
    'DONE',
    NULL
)
ON CONFLICT (TS_RAW_NO) DO NOTHING;

INSERT INTO TB_TS_FACT (
...
ON CONFLICT (TS_FACT_NO) DO NOTHING;
...
(
    ...
    '{"lookback_days":365,"target_horizon_days":1,"metrics":["PV_GEN_KW","LOAD_KW"],"exogenous":["IRRADIANCE","TEMP_C"]}'::jsonb,
    '2026-04-23 03:00:00',
    'RUN',
    'Y',
    'N'
...
(
    ...
    '{"required_params":["target_ymd"],"optional_params":["rtu_no","meter_no"]}'::jsonb,
    '2026-04-23 04:00:00',
    'RUN',
    'Y',
    'N'
...
ON CONFLICT (JOB_RUN_NO) DO NOTHING;
...
(
    ...
    'ML',
    'D+1 발전/소비량 예측 모델',
    '{"objective":"regression","n_estimators":500,"learning_rate":0.03}'::jsonb,
    'Y',
    'N'
)
ON CONFLICT (ALGM_NO) DO NOTHING;

INSERT INTO TB_MODEL_VER (
...
(
    ...
    '{"train_period":"2025-04-01~2026-03-31"}'::jsonb
)
ON CONFLICT (MODEL_VER_NO) DO NOTHING;

INSERT INTO TB_MODEL_EVAL (
...
(
    ...
    'MV_GMSC_202604',
    '2026-04-23 03:45:00',
    '{"PV_GEN_KW":{"MAPE":6.2},"LOAD_KW":{"MAPE":4.8}}'::jsonb,
    'Y',
    '목표 성능 기준 충족'
)
ON CONFLICT (MODEL_EVAL_NO) DO NOTHING;
...
ON CONFLICT (DEPLOY_NO) DO NOTHING;

INSERT INTO TB_FCST_RSLT (
...
ON CONFLICT (FCST_RSLT_NO) DO NOTHING;
...
ON CONFLICT (API_SVC_NO) DO NOTHING;
...
ON CONFLICT (BIZ_API_KEY_NO) DO NOTHING;

INSERT INTO TB_BIZ_API_KEY_ISSU_HIS (
...
ON CONFLICT (BIZ_API_KEY_ISSU_HIS_NO) DO NOTHING;

INSERT INTO TB_OPEN_API_CLNT (
...
ON CONFLICT (OPEN_API_CLNT_NO) DO NOTHING;

INSERT INTO TB_OPEN_API_KEY (
...
ON CONFLICT (OPEN_API_KEY_NO) DO NOTHING;

INSERT INTO TB_OPEN_API_KEY_SVC (
...
ON CONFLICT (OPEN_API_KEY_SVC_NO) DO NOTHING;

INSERT INTO TB_API_REQ_LOG (
...
ON CONFLICT (API_REQ_LOG_NO) DO NOTHING;
...
(
    ...
    'BK_GMSC_PM_001',
    'CREATE',
    '{"message":"광명스마트시티 PM 키 발급"}'::jsonb
)
ON CONFLICT (AUDIT_LOG_NO) DO NOTHING;
