...
본 `memory-bank/` 폴더는 **EnerBrain** 프로젝트(에너지 데이터 분석·AI·FastAPI 백엔드)의 세션 간 기억을 담는다. **제품 범위·스택·진행 상황**은 반드시 `projectbrief.md` → `productContext.md` → `systemPatterns.md` / `techContext.md` → `activeContext.md` → `progress.md` 순으로 확인한다. 아래 내용은 Cursor 에이전트용 **워크플로 규칙**(범용)이다.
